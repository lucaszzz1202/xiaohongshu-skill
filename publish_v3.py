...
with sync_playwright() as p:
    ...
    # 等待编辑器加载
    print('4. 等待编辑器加载...')
    page.wait_for_selector('textarea[placeholder="输入标题"]', state='visible', timeout=30000)
    print('   编辑器已加载!')
    ...
    sleep(2)
    page.screenshot(path=f'{out_dir}/pub3_content.png')

    print('7. 一键排版...')
    page.click('text=一键排版')
    sleep(3)
    page.screenshot(path=f'{out_dir}/pub3_format.png')
    ...
    if next_btn.count() > 0:
        ...
        sleep(12)
        page.screenshot(path=f'{out_dir}/pub3_next.png')
        
        pub_btn2 = page.locator('button:has-text("发布")')
        if pub_btn2.count() > 0:
            print('   找到发布按钮，点击...')
            pub_btn2.last.click()
            sleep(20)
    elif pub_btn.count() > 0:
        print('   直接找到发布按钮，点击...')
        pub_btn.last.click()
        sleep(20)
    else:
    ...
    page.screenshot(path=f'{out_dir}/pub3_final.png')
    ...
    if 'published=true' in current_url:
        ...
        sleep(15)
        current_url = page.url
        print(f'再次检查: {current_url}')
        if 'published=true' in current_url:
            print('发布成功!')
        else:
            body = page.text_content('body')
            print(f'页面文本: {body[:300]}')
