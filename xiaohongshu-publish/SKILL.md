...
version: 3.1.0
description: 小红书长文发布自动化工具（支持AI封面生成）
metadata: {"category":"social","platform":"xiaohongshu"}
updated: 2026-02-17
changelog: "v3.1.0 - 新增封面图生成能力（豆包Seedream 4.5），支持自定义封面"
---
...
3. 封面生成（可选）：豆包 Seedream 4.5 API（见 TOOLS.md 中的配置）

## 🎨 封面图生成（v3.1 新增）

小红书是视觉平台，好封面 = 高点击率。现在支持用 AI 生成封面图。
...
resp = requests.post(
    ...
    headers={
        ...
        "Authorization": "Bearer <API_KEY>"  # 见 TOOLS.md
    },
    json={
        "model": "doubao-seedream-4-5-251128",
        "prompt": "封面描述，中文效果好",
        "size": "1728x2304",  # 3:4 小红书封面最佳比例
        "response_format": "url",
        "watermark": False,
        "sequential_image_generation": "disabled"
    }
)
image_url = resp.json()["data"][0]["url"]  # 24小时有效，及时下载
```

### 封面设计原则
- 比例：3:4（1728x2304）在信息流中占面积最大，点击率最高
- 风格：干净、高对比度、主体突出，虾堡配色（暖橙红+金黄）
- 直接在 prompt 中描述需要的标题文字和关键词标签，Seedream 4.5 会直接生成在图中
- 与帖子内容相关，不要纯装饰
- **禁止出现小红书 logo 或任何平台标识**
- **IP一致性**：每次生成封面时，用 `xiabao_ip_reference.jpeg`（项目根目录）作为参考图传入 image 参数，保持虾堡形象统一

### 推荐尺寸
| 用途 | 宽高比 | 尺寸 |
|------|--------|------|
| 小红书封面（推荐） | 3:4 | 1728x2304 |
| 小红书竖图 | 9:16 | 1440x2560 |
| 正方形 | 1:1 | 2048x2048 |
| 横图 | 16:9 | 2560x1440 |

### 高级用法
- 组图模式：`sequential_image_generation="auto"`，一次生成多张关联图（最多15张）
- 图生图：`image` 参数传 URL 或 base64，可做风格统一的系列封面
- 提示词优化：`optimize_prompt_options.mode="standard"` 提升画质
